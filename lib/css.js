...
// hide element
api.hide =function( elem) {
	if( !api.visible( elem))
		// element is not visible
		return;
	
	// store current display property
	lib.attr.set( elem, '__old_display', api.getOne( elem, 'display'));
...
api.show =function( elem) {
	...
	// obtain previously stored display property
	if(( oldDisplay =lib.attr.get( elem, '__old_display')) ===undefined) {
		// property not found, use default display property
		oldDisplay =api.getDefaultDisplay( elem.nodeName);
		
	} else {
		// remove property from element
		lib.attr.remove( elem, '__old_display');
	}
