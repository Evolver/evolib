...
if( lib.data ===undefined)
	throw new ErrorException( 'lib.data is required by css.js');
...
api.set =function( elem, properties) {
	...
	if( lib.event !==undefined) {
		// fire 'css' event for the element
		lib.event.trigger( elem, 'css', properties);
	}
};
...
api.setStyle =function( elem, css) {
	...
	// see if events are supported
	var hasEvents =lib.event !==undefined;
	
	if( hasEvents) {
		// get current attribute's style
		var style =api.get( elem);
	}
	...
	if( hasEvents) {
		// get new attribute's style
		var newStyle =api.get( elem);
		
		// compute style diff
		for( var k in style)
			if( style[ k] ===newStyle[ k])
				delete newStyle[ k];
		
		if( !lib.empty( newStyle))
			// fire css event
			lib.event.trigger( elem, 'css', newStyle);
	}
};
...
api.viewport =function( elem) {
	...
	if( elem ===elem.ownerDocument.documentElement && elem.ownerDocument.body !==undefined) {
		// some browsers use document.documentElement as main scrolling element,
		//  some use document.body.
		if( elem.ownerDocument.body.scrollTop >0)
			scrollTop =elem.ownerDocument.body.scrollTop;
		if( elem.ownerDocument.body.scrollLeft >0)
			scrollLeft =elem.ownerDocument.body.scrollLeft;
	}
	...
	return {
		...
		'rect': {
			'top': scrollTop,
			'left': scrollLeft,
			'bottom': scrollTop +elem.clientHeight,
			'right': scrollLeft +elem.clientWidth
		}
	};
};
...
api.computeOffsetForView =function( elem, margin, alignX, alignY, viewportElement) {
	// default args
	if( margin ===undefined)
		margin =0;
	if( viewportElement ===undefined)
	...
	var style =api.style( elem, ['width', 'height']);
	var pos =api.position( elem, undefined, viewportElement);
	var vp =api.viewport( viewportElement);
	var offset =api.offset( elem);
	
	if( lib.isObject( margin)) {
		// multiple margins specified
		if( margin.top !==undefined)
			vp.rect.top +=margin.top;
		if( margin.left !==undefined)
			vp.rect.left +=margin.left;
		if( margin.bottom !==undefined)
			vp.rect.bottom -=margin.bottom;
		if( margin.right !==undefined)
			vp.rect.right -=margin.right;
		
	} else {
		// single margin specified
		vp.rect.top +=margin;
		vp.rect.bottom -=margin;
		vp.rect.left +=margin;
		vp.rect.right -=margin;
	}
	...
	var diff ={
		...
		'bottom': vp.rect.bottom - (pos.top +style.height),
		'left': pos.left -vp.rect.left,
		'right': vp.rect.right - (pos.left +style.width)
	};
...
// move positioned element into view
api.moveIntoView =function( elem, margin, alignX, alignY) {
	// compute view
	var ret =api.computeOffsetForView( elem, margin, alignX, alignY);
	
	// update offsets
	api.offset( elem, ret.offset);
};
