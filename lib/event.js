...
function isEventSupported( elem, eventType) {
  ...
  // onXXXX attribute and see what type is ot after creation.
  ...
  if( elem.setAttribute ===undefined) {
    ...
    result =(elem[ handlerName] ='return false;');
...
var events =api.nativeEvents ={
  ...
  'resize': {
    ...
    'bubbles': true,
    'cancelable': false,
    'supported': isEventSupported( frameElement, 'resize')
  },
  'scroll': {
    'type': 'HTMLEvents',
    'interface': 'Event',
...
api.bind =function( elem, type, handler, capture) {
  ...
  if( events[ type] ===undefined || !events[ type].supported) {
    ...
    if( customEvent.prototype.bind !==undefined) {
      // call bind handler
      customEventInstance.bind( elem, handler, capture);
    }
...
api.custom.add =function( event, handlingInstance) {
  ...
  if( events[ event] !==undefined && events[ event].supported)
    throw new ErrorException( 'Can not define custom event "' +event +'" because native event under the same name exists and is supported by the browser');
