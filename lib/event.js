...
var events ={
  ...
  'click': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':true},
  'dblclick': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':true},
  'mousedown': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':true},
  'mouseup': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':true},
  'mouseover': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':true},
  'mousemove': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':false},
  'mouseout': {'type':'MouseEvents','interface':'MouseEvent','bubbles':true,'cancelable':true},
  'mouseenter': {'type':'MouseEvents','interface':'MouseEvent','bubbles':false,'cancelable':true},
  'mouseleave': {'type':'MouseEvents','interface':'MouseEvent','bubbles':false,'cancelable':true},
  
  // HTML event types
  //  http://www.w3.org/TR/DOM-Level-2-Events/events.html#Events-eventgroupings-htmlevents
  'load': {'type':'HTMLEvents','interface':'Event','bubbles':false,'cancelable':false},
  'unload': {'type':'HTMLEvents','interface':'Event','bubbles':false,'cancelable':false},
  'abort': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  'error': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  'select': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  'change': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  'submit': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':true},
  'reset': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  'resize': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  'scroll': {'type':'HTMLEvents','interface':'Event','bubbles':true,'cancelable':false},
  
  // UI event types
  //  http://www.w3.org/TR/DOM-Level-2-Events/events.html#Events-UIEvent
  'DOMFocusIn': {'type':'UIEvents','interface':'UIEvent','bubbles':true,'cancelable':false},
  'DOMFocusOut': {'type':'UIEvents','interface':'UIEvent','bubbles':true,'cancelable':false},
  'DOMActivate': {'type':'UIEvents','interface':'UIEvent','bubbles':true,'cancelable':true},
  'focus': {'type':'UIEvents','interface':'UIEvent','bubbles':false,'cancelable':false},
  'focusin': {'type':'UIEvents','interface':'UIEvent','bubbles':true,'cancelable':false},
  'focusout': {'type':'UIEvents','interface':'UIEvent','bubbles':true,'cancelable':false},
  'blur': {'type':'UIEvents','interface':'UIEvent','bubbles':true,'cancelable':false},
  
  // Mutation event types
  //  http://www.w3.org/TR/DOM-Level-2-Events/events.html#Events-eventgroupings-mutationevents
  'DOMSubtreeModified': {'type':'MutationEvents','interface':'MutationEvent','bubbles':true,'cancelable':false},
  'DOMNodeInserted': {'type':'MutationEvents','interface':'MutationEvent','bubbles':true,'cancelable':false},
  'DOMNodeRemoved': {'type':'MutationEvents','interface':'MutationEvent','bubbles':true,'cancelable':false},
  'DOMNodeRemovedFromDocument': {'type':'MutationEvents','interface':'MutationEvent','bubbles':false,'cancelable':false},
  'DOMNodeInsertedIntoDocument': {'type':'MutationEvents','interface':'MutationEvent','bubbles':false,'cancelable':false},
  'DOMAttrModified': {'type':'MutationEvents','interface':'MutationEvent','bubbles':true,'cancelable':false},
  'DOMCharacterDataModified': {'type':'MutationEvents','interface':'MutationEvent','bubbles':true,'cancelable':false},
  
  // Keyboard event types
  //  http://www.w3.org/TR/2009/WD-DOM-Level-3-Events-20090908/#events-keyboardevents
  'keydown': {'type':'KeyboardEvents','interface':'KeyboardEvent','bubbles':true,'cancelable':true},
  'keyup': {'type':'KeyboardEvents','interface':'KeyboardEvent','bubbles':true,'cancelable':true},
  'keypress': {'type':'KeyboardEvents','interface':'KeyboardEvent','bubbles':true,'cancelable':true}
...
api.Event.prototype ={
  ...
  copyEvent: function( e) {
    ...
    // get context info from prototype
    var context =this.prototype.context;
  ...
  copyW3CEvent: function( e) {
    ...
    var context =getEventContext( this.type);
      ...
      switch( k =context[i]) {
        ...
        case 'button':
          ...
          if( e.button &0x2)
            ...
            // IE says no mouse button is pressed, weird.
            this.button =0;
        break;
        ...
        case 'relatedTarget':
          // FIXME (test if this is valid for onmouseover and onmouseout)
        ...
        default:
          throw new ErrorException( 'Don\'t know what to assign to context property "' +k +'" while copying context from IE event');
        break;
      }
    }
...
api.getData =function( elem, forceCreate) {
  ...
  var data =lib.getData( elem, '__eventlib');
  if( data ===undefined && forceCreate) {
    // initialize element
    lib.setData( elem, '__eventlib', data ={
...
api.removeData =function( elem) {
  // remove
  lib.removeData( elem, '__eventlib');
};
...
api.bind =function( elem, type, handler, capture) {
  ...
  if( events[ type] ===undefined) {
    ...
    if( customEvents[ type] ===undefined)
      // no custom event implementations are defined
      return;
      
    // see if attach method exists for this custom event
  ...
  if( eventModel ==EventModels.W3C) {
    ...
    var handler =function( nativeEvent /* native event object */){
      ...
      api.debug( 'native event incoming ' +nativeEvent.target);
      
      // do not propagate native event
      nativeEvent.stopPropagation();

      // ignore of the event requested?
      if( nativeEvent.alreadyProcessed || dontProcessNextInternal) {
      ...
      // BUG: When binding to window and document, or window
      //  and some else element, Opera will trigger event TWICE.
      //  To workaround this, we implement safety checking var
      //  .alreadyProcessed. Mark native event as already processed.
      //  And if event comes in one more time, ignore it.
      ...
      // BUG: Safari, Chrome, Mozilla: events triggered on
      //  window object have .target ==document. To workaround
      //  this, we will check if event bubbles. If event does
      //  not bubble, it means that event could not be triggered
      //  from the down of the DOM tree, so, the event target
      //  technically can't be anything other than 'elem'.
      //  For example, when window's LOAD event triggers, native
      //  event object has .target ==document, where it shouldn't
      //  have it.
      if( !e.bubbles)
        e.target =elem;
    ...
    // if event listener attachment succeeds, this will be set to true.
    var added =false;
    ...
    try {
      ...
      alert( 'Exception during addEventListener: ' +e +'. This should never happen.');
    }
    ...
    // this will be set to true if event handler addition succeeds
    var added =false;
    ...
    try {
      ...
      alert( 'Exception during attachEvent: ' +e);
    }
    ...
    throw new ErrorException( 'Unsupported event model');
};
...
api.unbind =function( elem, type, handler, capture) {
  ...
  function unbindHandler( type, typeHandlers, handler) {
    ...
    if( isCustom && customEventInstance !==undefined && customEventInstance.unbind !==undefined)
      customEventInstance.unbind( elem, handler, capture);
  ...
  for( k in data.customEvents) {
    ...
    if( data.bubble[ k] ===undefined && data.capture[k] ===undefined) {
      ...
      if( customEventInstance.detach !==undefined)
        customEventInstance.detach( elem);
    }
  }
  ...
  for( k in data.handlers) {
    ...
    if( eventModel ==EventModels.IE)
      ...
      throw new ErrorException( 'Unsupported event model "' +eventModel +'"');
...
api.triggerEvent =function( elem, eventObject, data, internal, capturer) {
  ...
  var elemWindow =lib.getElementWindow( elem);
  if( elemWindow ===undefined)
    throw new ErrorException( 'Unable to determine element\'s window object. Make sure you are using DOMElements as event targets.');
      ...
      if( nodeData !==undefined && nodeData.capture[ type] !==undefined && nodeData.capture[ type].length > 0) {
        ...
        for( var h =0; h < handlers.length; ++h) {
          try {
            ...
            //  FIXME: figure out how to issue a warning on the exception.
            alert( 'Exception during capturing: ' +e);
          }
  ...
  for( i =0; i < path.length; ++i) {
    ...
    if( nodeData !==undefined && nodeData.bubble[ type] !==undefined && nodeData.bubble[ type].length > 0) {
      ...
      for( var h =0; h < handlers.length; ++h) {
        try {
          ...
          // handler has thrown an exception.
          //  FIXME: figure out how to issue a warning on the exception.
          alert( 'Exception during bubbling: ' +e +' (' +handlers[h] +')');
        }
    ...
    if( eventModel ==EventModels.IE) {
      if( internal) {
        ...
        if( !lib.isDescendantOf( node, capturer, true))
        ...
        if( isNativeEvent) {
          ...
          if( node[ handlerName]) {
            ...
            try {
              ...
              // error in handler
              alert( 'Exception in ' +handlerName +' handler: ' +e +' (' +node[handlerName] +')');
            }
          ...
          // firefox fails to register node[onXXXX] for BODY element
            ...
            try {
              ...
              // error in handler
              alert( 'Exception in ' +type +' handler: ' +e +' (' +node[handlerName] +')');
            }
          }
        }
      }
        ...
        if( eventModel ==EventModels.W3C && !clickTriggerable && elem.nodeName && elem.nodeName =='A') {
          ...
          if( href !==null) {
            ...
            if( href.substr( 0, 11) =='javascript:') {
              ...
              lib.getElementWindow( elem).location.href =href;
            }
          }
...
api.triggerNativeEvent =function( elem, type) {
  ...
  if( eventModel ==EventModels.W3C) {
    ...
    if( eventInfo.type =='MouseEvents') {
      ...
      throw new ErrorException( 'Unable to initialize event of type "' +eventInfo.type +'"');
    ...
    throw new ErrorException( 'Unsupported event mode "' +eventModel +'"');
  
  return ret;
};

// proxy event
api.proxy =function( eventObject, newTarget) {
...
api.custom.add =function( event, handlingInstance) {
  ...
  if( events[ event] !==undefined)
    throw new ErrorException( 'Can not define custom event "' +event +'" because native event under the same name exists');
  // see if event already exists
  if( customEvents[ event] !==undefined)
    throw new ErrorException( 'Custom event "' +event +'" already exists');
