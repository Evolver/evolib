  ...
  for( i =0; i < elems.length; ++i) {
    ...
    for( p in entry.props) {
      ...
      if( tq ===undefined) {
        ...
        // animate property
        propTransition[ p] =api.getPropTransitionCSSValue( p, tq.current);
        hasTransitions =true;
      }
    }
...
api.transform =function( elem, prop, from, to, speed, easing, tickCallback, completionCallback) {
  ...
  if( lib.isString( prop)) {
    ...
    if( from ===undefined || from ===null)
      from =api.getPropTransitionRawCSSValue( prop, lib.css.getOne( elem, prop));
    ...
    for( k in prop) {
      ...
      if( prop[k].from ===undefined || prop[k].from ===null)
        props.push[ k];
    }
    ...
    for( k in prop) {
      ...
      AddTransition( elementInfo, k,
        entry.from ===undefined || entry.from ===null ? style[ k] : entry.from,
        entry.to,
        entry.speed,
        entry.easing,
        entry.tickCallback,
...
api.getPropTransitionCSSValue =function( property, rawValue) {
  // see which property is passed in
  switch( property) {
    case 'width':
    case 'height':
